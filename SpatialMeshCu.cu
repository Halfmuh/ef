...
__global__ void SetBoundaryConditionsSlow(double* potential, int3* d_n_nodes, double* d_boundary) {
	...
	if (mesh_idx.x == d_n_nodes->x - 1) {
		potential[flat_idx] = d_boundary[LEFT];
	}
	if (mesh_idx.y == 0)
	{
		potential[flat_idx] = d_boundary[BOTTOM];
	}
	if (mesh_idx.y == d_n_nodes->y - 1) {
		potential[flat_idx] = d_boundary[TOP];
	}
	if (mesh_idx.z == 0)
	{
...
void SpatialMeshCu::copy_boundary_to_device(Config &conf) {
	...
	cuda_status = cudaMemcpy(d_boundary, (const void*)&boundary, sizeof(double) * 6, cudaMemcpyHostToDevice);
	cuda_status_check(cuda_status, debug_message);
}
